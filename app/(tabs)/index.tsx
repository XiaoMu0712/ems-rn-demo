...
const StatusStatCard: React.FC<StatusStatCardProps> = ({ status, count, onPress }) => {
  const theme = useTheme();
  ...
  return (
    <View style={[styles.statusStatCard, { backgroundColor: '#FFFFFF' }]} onTouchEnd={onPress}>
      <View style={[styles.statusStatIconContainer, { backgroundColor: statusInfo.color + '20' }]}>
...
const TimeStatCard: React.FC<TimeStatCardProps> = ({ timeRange, count, onPress }) => {
  const theme = useTheme();
  ...
  const getTimeInfo = (timeRange: string) => {
    switch (timeRange) {
      ...
      case 'thisMonth':
        return { icon: 'calendar-month', color: '#4CAF50', label: 'This Month' };
      default:
        return { icon: 'help', color: '#757575', label: 'Unknown' };
    }
  };
  ...
  return (
    <View style={[styles.timeStatCard, { backgroundColor: '#FFFFFF' }]} onTouchEnd={onPress}>
      <View style={[styles.timeStatIconContainer, { backgroundColor: timeInfo.color + '20' }]}>
        <MaterialIcons name={timeInfo.icon as any} size={20} color={timeInfo.color} />
      </View>
...
const ReportPreviewItem: React.FC<ReportPreviewItemProps> = ({ report, onPress }) => {
  const theme = useTheme();
      ...
      <Card.Content>
        <View style={styles.reportPreviewHeader}>
          <View>
            ...
            <Text style={styles.reportPreviewMeta}>{report.date} • {report.expenseCount} expenses</Text>
          </View>
          <Chip 
            style={[styles.reportPreviewStatusChip, { backgroundColor: getStatusColor(report.status) }]}
        ...
        <View style={styles.reportPreviewFooter}>
          <Text style={styles.reportPreviewAmount}>${report.amount.toFixed(2)}</Text>
...
const ApprovalPreviewItem: React.FC<ApprovalPreviewItemProps> = ({ report, onPress }) => {
  const theme = useTheme();
      ...
      <Card.Content>
        <View style={styles.approvalPreviewHeader}>
          <View>
            ...
            <Text style={styles.approvalPreviewMeta}>Submitted by {report.submittedBy}</Text>
          </View>
          <Chip 
        ...
        <View style={styles.approvalPreviewFooter}>
          <Text style={styles.approvalPreviewAmount}>${report.amount.toFixed(2)}</Text>
          <MaterialIcons name="chevron-right" size={20} color={theme.colors.onSurfaceVariant} />
        </View>
      </Card.Content>
    </Card>
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      >
        ...
        <View style={styles.header}>
          <Title style={[styles.welcomeTitle, { color: theme.colors.onSurface }]}>Expense Dashboard</Title>
        ...
        <View style={styles.sectionContainer}>
          <View style={styles.sectionHeader}>
            <Title style={[styles.sectionTitle, { color: theme.colors.onSurface }]}>My Reports</Title>
            <Button
              mode="text"
              onPress={handleNavigation.viewAllReports}
              compact
              labelStyle={[styles.viewAllButtonText, { color: theme.colors.primary }]}
            >
              View All
            </Button>
          </View>
          ...
          <View style={styles.recentReportsContainer}>
            <Title style={[styles.subsectionTitle, { color: theme.colors.onSurface }]}>Recent Reports</Title>
        ...
        <View style={styles.sectionContainer}>
          <View style={styles.sectionHeader}>
            <Title style={[styles.sectionTitle, { color: theme.colors.onSurface }]}>My Approvals</Title>
            <Button
              mode="text"
          ...
          <View style={styles.recentApprovalsContainer}>
            <Title style={[styles.subsectionTitle, { color: theme.colors.onSurface }]}>Recent Approvals</Title>
